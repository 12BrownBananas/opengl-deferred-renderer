...
PointLight::PointLight(glm::vec3 position, glm::vec3 lightColor, glm::vec2 cubemapDimensions) : Light(position, lightColor) {
	...
	for (unsigned int i = 0; i < 6; ++i)
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_DEPTH_COMPONENT, cubemapDimensions.x, cubemapDimensions.y, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
	...
	shadowProj = glm::perspective(glm::radians(90.0f), (float)cubemapDimensions.x / (float)cubemapDimensions.y, nearPlane, farPlane);
